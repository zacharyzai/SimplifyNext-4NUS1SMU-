...
- [ ] **Tracing/audit log entry?** — every consequential decision the feature makes appends one record to `CashFlowState["trace"]` in the shared shape every module already uses (`node`, `ts`, `checked`, `found`, `concluded`, `confidence`, `degraded` — see `shared/schema.py` section 6.2), with `checked` and `concluded` populated — no empty/placeholder reasoning strings.
