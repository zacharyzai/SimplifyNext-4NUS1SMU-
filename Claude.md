...
- **Extended again 2026-09-06/07** with `recurring_bills: List[dict]` for the chat box's new bill intent (see below). Same category of change as the raw-input fields above: pure accumulated state, only read by `modules/forecast.py`, written by `server.py` after classification.
...
- The chat box's free text is classified (EARNINGS / BILL / OTHER) in `server.py` via `shared/llm.py`'s `converse()` before it ever reaches `graph.py` — same "LLM transcribes, Python decides" pattern as ingestion's `extract_from_text`. Every chat-log acknowledgement the user sees is a deterministic Python template, never the model's own words, so a misclassification can at worst produce "I didn't understand that," never a wrong stored number. The trace panel's "checked" strings are humanized client-side only (`static/app.js`'s `CHECKED_HUMANIZERS`) — backend trace content is untouched so module self-tests asserting on it verbatim still pass.
